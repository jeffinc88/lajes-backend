...
- **Deploy:** Railway (auto-deploy via push na branch `main` do GitHub — usuário `jeffinc88`)
...
- **Nunca expor senhas, tokens ou chaves de API no código** — variáveis de ambiente ficam no Railway
...
- ❌ Não fazer deploy automático — o Railway já cuida disso via push na main
...
Ficam configuradas no Railway. Para rodar localmente, criar `.env` com:

```
DATABASE_URL=postgresql://postgres.aewqpdxraiibypqsirxp:J3ff1ncarvalho123@aws-1-us-east-1.pooler.supabase.com:6543/postgres
JWT_SECRET=lajes_secret_key_2024
PORT=3000
ANTHROPIC_API_KEY=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
...
3. Railway detecta o push e faz deploy automático do backend
...
- [ ] Outras variáveis de ambiente além de `DATABASE_URL`, `JWT_SECRET` e `PORT` — veja no Railway → Variables